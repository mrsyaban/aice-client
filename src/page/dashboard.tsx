  ...
  return (
    <>
      ...
      <div className="flex flex-col pb-12 gap-16 w-full bg-primary-white">
        {/* features */}
        <div className="flex flex-row w-full overflow-x-auto pb-12">
          <div className="flex flex-row gap-24 min-w-[1561px] w-full px-24 justify-center pt-12">
            <FeatureCard
        ...
        <div className="flex flex-row gap-12 px-20">
          {/* jobs */}
          <div className="flex flex-col h-fit gap-8 items-center w-full max-w-[1000px] mx-auto bg-slate-300 p-6 rounded-lg">
            <div className="text-3xl font-bold text-primary-blue w-full text-start">Job Histories</div>
            {jobs.length > 0 ? (
              <div className="flex flex-col gap-6 w-full ">
                {jobs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((job, idx) => (
          ...
          <div className="flex flex-col h-fit gap-8 items-center w-full max-w-[1000px] mx-auto bg-slate-300 p-6 rounded-lg">
            ...
            {interviews.length === 0 ? (
              ...
              <div className="flex flex-col gap-6 w-full max-w-[1000px]">
                {interviews.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((interview, idx) => (
                  <div key={idx} className="flex flex-row w-full py-4 px-6 bg-white rounded-lg gap-4 justify-between">
                    <div className="flex flex-col justify-between gap-2">
